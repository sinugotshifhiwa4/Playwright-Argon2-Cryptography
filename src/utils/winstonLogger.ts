...
class WinstonLogger extends LoggerUtils {
  // The logger instance
  ...
  private static initializeLogger(): void {
    try {
      const loggerDir = this.resolvePath(
        process.cwd(),
        WinstonLoggerConfig.DIR_PATH
      );
      ...
      const infoTransport = this.createInfoTransport(
        timestampFormat,
        customFormat,
        loggerDir
      );
      ...
      const errorTransport = this.createErrorTransport(
        timestampFormat,
        customFormat,
        loggerDir
      );
      ...
      const warnTransport = this.createWarnTransport(
        timestampFormat,
        customFormat,
        loggerDir
      );
...
// Get the logger instance and export it
